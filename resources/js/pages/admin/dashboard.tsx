...
import OverloadUser from './overload';
import NonOverloadUser from './unoverloaded';
...
export default function AdminDashboard({
    ...
    const occupied = users.filter((user) => user?.total_projects > 3);
    const unoccupied = users.filter((user) => user?.total_projects < 3);

// console.log(users,occupied, unoccupied)
            ...
            <div className="mt-10 grid h-[300px] max-h-[300px] grid-cols-1 gap-4 overflow-hidden sm:grid-cols-3 md:gap-0">
                <div className="bg-sidebar grid gap-2 p-2 md:grid-cols-2 dark:bg-neutral-900">
                    <div className="grid gap-2">
                        <OverloadUser count={occupiedStaff} totalP={totalProjects} occupied={occupied}  />
                        <NonOverloadUser count={unoccupiedStaff} totalP={totalProjects}  nonOccupied={unoccupied} />
                    </div>
                    <div className="grid gap-2">
                        <div className="group grid cursor-pointer rounded-md bg-gray-200 p-2 dark:bg-black/50">
                            ...
                            <div className="flex">
                                <p className="mt-auto text-2xl font-bold sm:text-3xl dark:text-neutral-500">
                        ...
                        <div className="group grid cursor-pointer rounded-md bg-gray-200 p-2 dark:bg-black/50">
                            ...
                            <div className="flex">
                                <p className="mt-auto text-2xl font-bold sm:text-3xl dark:text-neutral-500">
                                    {deactivatedProjects}/{totalProjects}
                                </p>
                            </div>
                        </div>
                    </div>
