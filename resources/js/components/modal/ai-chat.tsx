...
const AiChat = () => {
    ...
    const sendMessage = useCallback(async () => {
        ...
        try {
            const response = await axios.post('/gemini-query', { prompt: message });
    ...
    return (
        <>
            <button
                className="fixed right-4 bottom-4 m-0 inline-flex h-16 w-16 cursor-pointer items-center justify-center rounded-full border border-gray-200 bg-black bg-none p-0 text-sm leading-5 font-medium normal-case hover:bg-gray-700 hover:text-gray-900 disabled:pointer-events-none disabled:opacity-50"
                type="button"
            ...
            <div
                style={{ boxShadow: '0 0 #0000, 0 0 #0000, 0 1px 2px 0 rgb(0 0 0 / 0.05)', display: chatVisible ? 'block' : 'none' }}
                className="fixed right-0 bottom-[calc(4rem+1.5rem)] mr-4 h-[634px] w-[440px] rounded-lg border border-[#e5e7eb] bg-white p-6"
            >
                ...
                <div className="h-[474px] pr-4" style={{ minWidth: '100%', display: 'block' }}>
                    ...
                    <div className="my-4 flex flex-1 gap-3 text-sm text-gray-600">
                        ...
                        <p className="leading-relaxed">
                            <span className="block font-bold text-gray-700">AI </span>Sorry, I couldn't find any information in the documentation
                            about that. Expect answer to be less accurateI could not find the answer to this in the verified sources.
                        </p>
                    </div>
                </div>
